...
@section('content')
    <div class="bg-white rounded-lg shadow p-4">
        <h2 class="text-lg font-semibold mb-4">Nuevo Campo de Micronegocio</h2>

        <form method="POST" action="{{ route('admin.microbusiness-fields.store') }}" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <select name="field_type" class="w-full border rounded-md px-3 py-2" required>
                    @foreach (['text','textarea','number','select','checkbox','date','email','url'] as $type)
                        <option value="{{ $type }}" @selected(old('field_type') === $type)>{{ $type }}</option>
                    @endforeach
                </select>
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Opciones (separadas por coma, para select/checkbox)</label>
                <input type="text" name="options_text" value="{{ old('options_text') }}" class="w-full border rounded-md px-3 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Orden</label>
                <input type="number" min="0" name="sort_order" value="{{ old('sort_order', 0) }}" class="w-full border rounded-md px-3 py-2">
            </div>
